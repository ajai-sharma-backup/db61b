...
class CommandInterpreter {
     ...
     *     }
     *
     * which combines the calls to .nextIs and .next.
     */
    ...
    Table selectClause() {
        ...
        ArrayList<Condition> conditions =
                conditionClause(tables.toArray(new Table[tables.size()]));
        switch (tables.size()) {
        case 1:
            return tables.get(0).select(columns, conditions);
        case 2:
            return tables.get(0).select(tables.get(1), columns, conditions);
        default:
            throw error("this really shouldn't happen ever");
        }
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        if (_input.nextIf("where")) {
            ...
            result.add(new Condition.NullCondition());
        }
        return result;
    }
